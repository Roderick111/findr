...
fn test_run_full_index_creates_db_and_content_index() {
    ...
    assert!(tp.content_index_path().exists(), "Content index should exist at final path");

    // No temp files left
    assert!(!tp.data().join("index.db.new").exists(), "Temp DB should be cleaned up");
    assert!(!tp.data().join("content_index.new").exists(), "Temp content index should be cleaned up");
    // No backup files left
    assert!(!tp.data().join("index.db.bak").exists(), "Backup DB should be cleaned up");
    assert!(!tp.data().join("content_index.bak").exists(), "Backup content index should be cleaned up");
    ...
    assert!(doc_count >= 3, "Content index should have at least 3 docs, got {}", doc_count);

    // Verify content_indexed_count metadata was set
    let stored_count = db.get_meta("content_indexed_count").unwrap();
    assert!(stored_count.is_some(), "content_indexed_count metadata should be set");
    let stored_count_num: usize = stored_count.unwrap().parse().unwrap();
    assert!(stored_count_num >= 3, "content_indexed_count should be >= 3, got {}", stored_count_num);
...
fn test_run_full_index_atomic_swap_replaces_previous() {
    ...
    assert!(count2 > count1, "Second index should have more files: {} vs {}", count2, count1);
...
fn test_run_incremental_index_detects_new_files() {
    ...
    // Add new files
    create_test_files(tp.source(), &[
        ("new1.txt", "new file one"),
        ("new2.txt", "new file two"),
    ]);
...
fn test_run_incremental_index_no_changes() {
    ...
    let count_after = db.file_count().unwrap();
    assert_eq!(count_before, count_after, "No changes should mean same file count");
}
...
fn test_reconcile_detects_drift_and_rebuilds() {
    ...
    assert!(stored.is_some(), "content_indexed_count should exist after full index");
    ...
    db.set_meta("last_reconcile_check", &old_time.to_rfc3339()).unwrap();
    ...
    assert!(new_count < 1000, "content_indexed_count should be corrected from 1000 to actual: {}", new_count);
    assert!(new_count >= 5, "Should still have at least 5 indexed files, got {}", new_count);
}
...
fn test_reconcile_skips_when_recent() {
    ...
    db.set_meta("last_reconcile_check", &chrono::Utc::now().to_rfc3339()).unwrap();
    ...
    assert_eq!(stored, "9999", "Reconcile should have skipped - count unchanged");
}
...
fn test_rebuild_hnsw_creates_index_files() {
    ...
    assert_eq!(count, Some("10".to_string()),
...
fn test_rebuild_hnsw_noop_with_empty_vectors() {
    ...
    let count = db.get_meta("hnsw_vector_count").unwrap();
    assert_eq!(count, None, "hnsw_vector_count should not be set for empty vectors");
}
...
fn test_rebuild_hnsw_replaces_existing_index() {
    ...
    for i in 5..10 {
        let vec: Vec<f32> = (0..EMBED_DIMS).map(|d| ((d + i) as f32 * 0.02).cos()).collect();
        let bytes = semantic::vec_to_bytes(&vec);
        more_entries.push((
            format!("/fake/file_{}.txt", i),
            bytes,
    ...
    assert_eq!(count2, Some("10".to_string()), "should reflect all 10 vectors after rebuild");
    assert!(semantic::hnsw_index_exists(tp.data()));
}
